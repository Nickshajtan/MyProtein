...
class HCC_Widget_Layered_Nav_Filters extends \WC_Widget_Layered_Nav_Filters {
  
    private function _get_url() : string {
      $this->_url  = explode('?', ( ((!empty($_SERVER['HTTPS'])) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] ) );
      return $this->_url[0];
    }
   ...
   /**
	...
	public function widget( $args, $instance ) {
		...
		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) ) {
            ...
            $this->_url = $this->_get_url();
            if( $this->_url ) {
              echo '<li><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="#" class="button d-block ml-auto mr-auto" onclick="(function(){ window.location.href = \'' . $this->_get_url() . '\';} )();">'. __('Очистить все', 'woocommerce') .'</a></li>';
            }
